                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" placeholder="Enter URL"
                                                name="URL" value="{{ @$result[0]->URL }}" />
                                        </div>
                                        @error('URL')
                                            <span style="color:red;"
                                                role="alert"><strong>{{ str_replace(' u r l', ' Url ', $message) }}</strong></span>
                                        @enderror
                                    </div>
                                </div>
                                <div class="box-footer row">
                                    @if (isset($result))
                                        <button type="submit" class="btn btn-info col-xs-5 col-md-2"><i
                                                class="fa fa-fw fa-upload"></i>Update</button>
                                        <a href="{{ route('style.templates') }}"
                                            class="btn btn-danger col-xs-5 col-md-2 mx-2"><i class="fa fa-fw fa-close"></i>
                                            Cancel</a>
                                    @else
                                        <button type="submit" class="btn btn-success col-xs-12 col-sm-3 col-md-2"><i
                                ...
                                <tbody class="fw-6 v-align-mid">
                                    ...
                                    @foreach ($data as $datas)
                                        ...
                                        <tr>
                                            ...
                                            <td class="d-flex">
                                                <a href="{{ url('IIN/styletempedit/' . base64_encode($datas->id)) }}"
                                                    class="btn btn-social-icon cst-edit" data-toggle="tooltip"
                                                    title="Edit"><i class="fa fa-fw fa-edit"></i></a>
                                                <a href="{{ url('IIN/styletempdel/' . base64_encode($datas->id)) }}"
                                                    class="btn btn-social-icon cst-del" data-toggle="tooltip" title="Delete"
